/*
  # Fix get_elderly_profile_full_details for Current Calls Schema

  ## Changes
  - Update calls query to use correct column names:
    - Use `created_at` instead of `started_at` for when call was initiated
    - Remove references to non-existent columns
  - Ensures function works with the actual calls table structure

  ## Impact
  - Fixes the function to return correct call history data
  - Enables webhook payload to include accurate recent call information
*/

-- Drop and recreate with correct calls schema
...
BEGIN
  ...
  -- Get recent call summaries (last 7 days) with correct schema
  SELECT jsonb_agg(
    jsonb_build_object(
      'call_id', c.id,
